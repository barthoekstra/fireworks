...
ppi_hrw_mass %>%
  ...
  mutate(vid_new = VIR / avg_mass,
         ...
         birds_smooth_log = log(birds_smooth)) -> ppi_hrw_mass_nonzero

ggplot(ppi_hrw_mass_nonzero, aes(x = pop_eff, y = VIR, col)) +
  geom_point(aes(colour = dist_urban, size = nr_birds)) +
  scale_color_viridis() +
  scale_x_continuous(trans = "log10")
```
...
m1 <- lm(log(VIR + 1) ~ log(birds_smooth + 1) + log(pop + 1) + dist_urban + landuse_class, data = ppi_hrw_mass_nonzero)
...
ppi_hrw_mass %>%
  ...
  drop_na() %>%
  sample_frac(0.25)-> ppi_hrw_mass_gam

mgam <- gam(log(VIR + 1) ~ s(log(birds_smooth + 1)) + s(log(pop_cor + 1)), data = ppi_hrw_mass_gam, method = "REML")
summary(mgam)
```
