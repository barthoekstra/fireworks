...
m1 <- lm(log(vir + 1) ~ log(birds_smooth + 1) + log(pop_cor + 1) + landuse_class, data = ppi_hrw_mass_nonzero)
...
ppi_hrw_mass %>%
  ...
  drop_na() %>%
  sample_frac(0.05)-> ppi_hrw_mass_gam

mgam <- gam(log(vir + 1) ~ s(log(birds_smooth + 1), k = 15) + s(log(pop_cor + 1), by = landuse_class, k = 15), data = ppi_hrw_mass_gam, method = "REML")
summary(mgam)
```
