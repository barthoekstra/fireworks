...
As usual, we use [bioRad](http://adokter.github.io/bioRad) [@dokter_biorad_2019], but this time we include `plotly` for some interactive plotting.
...
```{r plot_em_filtered_scans, warning=FALSE}
i = 1
...
side_by_side_ppi(pvol_hrw, pvol_dhl, "Herwijnen", "Den Helder", params = c("DBZH", "VRADH", "RHOHV", "BIOLRRHOHV", "BIOLDRHOHV", "DPR", "BIOLR", "BIOLD"))
```
...
We select `r dynamic_nr_preceding_scans` (`r dynamic_nr_preceding_scans / 12` hours worth of scans) preceding the fireworks event and add an additional margin of `r dynamic_time_margin` scans (`r dynamic_time_margin * 5` minutes of scans) as that the `VIR` plots in the previous chapter have shown numbers of birds aloft are very low and stable up to that period.
...
selected_scan_hrw <- sub("fireworks-2017-2018", "clutter-removal-20171231", pvol_hrw_path)
selected_scan_dhl <- sub("fireworks-2017-2018", "clutter-removal-20171231", pvol_dhl_path)
...
```{r calculate_stack_average_reflectivity}
...
```{r calculate_mean_static_reflectivities}
...
source("R/calculate_reflectivity_stack_mean.R") # Source ecause full_repro may be set to FALSE
...
side_by_side_ppi(pvol_clutter_static_hrw, pvol_clutter_dynamic_dhl, "Herwijnen static clutter", "Den Helder static clutter", 
                 params = "DBZH_AVG", range_max = 50000, scan_id = 1, basemap = TRUE, zlim = c(-11, -10))
```
...
vp_dhl <- calculate_vp(file = pvol_dhl_path, vpfile = paste("data/processed/vp/", basename(pvol_dhl_path), sep = ""), verbose = FALSE)

corrected_ppi_hrw <- integrate_to_ppi(pvol_hrw, vp_hrw, res = 500, xlim = c(-50000, 50000), ylim = c(-50000, 50000))
corrected_ppi_dhl <- integrate_to_ppi(pvol_dhl, vp_dhl, res = 500, xlim = c(-50000, 50000), ylim = c(-50000, 50000))
...
```{r map_range_corrected_herwijnen, fig.width=10, warning=FALSE}
mapview(filtered_corrected_ppi_hrw$data, alpha.regions = 0.6, col.regions = inferno, maxpixels=2000000,
        na.color = "#00000000", map.types = c("CartoDB.Positron", "CartoDB.DarkMatter", "Esri.WorldImagery"),
        layer.name = colnames(filtered_corrected_ppi_hrw$data@data))
```

### Range-corrected PPI of Den Helder radar

```{r map_range_corrected_den_helder, fig.width=10, warning=FALSE}
mapview(filtered_corrected_ppi_dhl$data, alpha.regions = 0.6, col.regions = inferno, maxpixels=2000000,
        na.color = "#00000000", map.types = c("CartoDB.Positron", "CartoDB.DarkMatter", "Esri.WorldImagery"),
        layer.name = colnames(filtered_corrected_ppi_dhl$data@data))
```
