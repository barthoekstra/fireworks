...
```{r verify_dpr_classifications, out.width='100%', warning=FALSE}
...
```{r verify_precipitation_removal, out.width='100%', warning=FALSE}
...
```{r verify_dynamic_clutter_map, out.width='100%', message=FALSE, warning=FALSE}
...
```{r verify_static_clutter_map, out.width='100%', message=FALSE, warning=FALSE}
...
```{r plot_corrected_vir, out.width='100%'}
...
*Unfortunately, due to [the way `leaflet` stores maps it produces](https://github.com/rstudio/bookdown/issues/15#issuecomment-573776659), we cannot plot them here using K-M knitting to generate this document. Run these chunks interactively and they should work fine.*

### Range-corrected PPI of Herwijnen radar

```{r map_range_corrected_herwijnen, out.width='100%', eval=FALSE}
mapview(corrected_ppi_hrw$data)
```

### Range-corrected PPI of Den Helder radar

```{r map_range_corrected_den_helder, out.width='100%', eval=FALSE}
...
## Preprocess baseline disturbance scans
...
```{r preprocess_disturbance_baseline_scans, eval=full_repro, results='hide'}
