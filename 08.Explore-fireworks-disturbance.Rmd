...
2. The total biomass of birds on the ground.
2. The average bodymass of the birds on the ground.
3. The take-off habitat of these birds.
4. The human population density in the vicinity of birds, corrected for the distance to urban areas.
...
We use the waterbird count from January 2018, and the point-transect counts from 2017, as these are the best in terms of coverage. We calculate the `nr_birds` and `total_biomass` parameters first, because we need to spread these out over the number of PPI pixels that are contained within a count when we join the counts and PPIs.
...
ptt_raw %>%
  ...
  summarise(nr_birds = mean(nr_birds),
            total_biomass = mean(total_biomass)) -> ptt
```
...
```{r}
ppi_hrw %>%
  left_join(dplyr::select(wb, area_nr, nr_birds, total_biomass), by = c("wb_area_nr" = "area_nr")) %>%
  rename(wb_nr_birds = nr_birds, wb_total_biomass = total_biomass) %>%
  group_by(datetime, wb_area_nr) %>%
  mutate(wb_nr_birds = wb_nr_birds / n(),  # This results in decimals for nrs of birds, but that's fine
         wb_total_biomass = wb_total_biomass / n(),
         wb_n = n()) %>%  
  ungroup() %>%
  left_join(dplyr::select(ptt, route, nr_birds, total_biomass), by = c("ptt_route" = "route")) %>%
  rename(ptt_nr_birds = nr_birds, ptt_total_biomass = total_biomass) %>%
  group_by(datetime, ptt_route) %>%
  mutate(ptt_nr_birds = ptt_nr_birds / n(),  # This results in decimals for nrs of birds, but that's fine
         ptt_total_biomass = ptt_total_biomass / n(),
         ptt_n = n()) %>% 
  ungroup() %>%
  filter_at(vars(wb_nr_birds, ptt_nr_birds), any_vars(!is.na(.))) %>%
  rowwise() %>%
  mutate(nr_birds = sum(wb_nr_birds, ptt_nr_birds, na.rm = TRUE),
         total_biomass = sum(wb_total_biomass, ptt_total_biomass, na.rm = TRUE)) %>%
  ungroup() -> hrw
...
We can also add the `avg_bodymass` of birds within a PPI pixel, which we calculate separately from a combination of the waterbird and point-transect counts. There is no need to 'split' this over the number of PPI pixels, as it is an average that we can assume applies throughout the area.

```{r}
calculate_average_bird_bodymass <- function(wb_area_nr, wb_year, ptt_route, ptt_year) {
  wb_raw %>%
    filter(area_nr == wb_area_nr, 
           year == wb_year) %>%
    dplyr::select(number, mean_weight) -> wb_temp
  
  ptt_raw %>%
    filter(route == ptt_route,
           year == ptt_year) %>%
    dplyr::select(number, mean_weight) -> ptt_temp
  
  wb_temp %>%
    bind_rows(ptt_temp) -> weights
  
  weighted.mean(weights$mean_weight, weights$number, na.rm = TRUE)
}

hrw %>%
  dplyr::select(wb_area_nr, ptt_route) %>%
  distinct() %>%
  rowwise() %>%
  mutate(avg_bodymass = calculate_average_bird_bodymass(wb_area_nr, wb_year, ptt_route, ptt_year)) %>%
  right_join(hrw, by = c("wb_area_nr" = "wb_area_nr", "ptt_route" = "ptt_route")) %>%
  ungroup() -> hrw
...
dhl %>%
  ...
  rowwise() %>%
  mutate(avg_bodymass = calculate_average_bird_bodymass(wb_area_nr, wb_year, ptt_route, ptt_year)) %>%
  right_join(dhl, by = c("wb_area_nr" = "wb_area_nr", "ptt_route" = "ptt_route")) %>%
  ungroup() -> dhl
```

Now, finally, all the relevant variables are present in the PPIs.

## Preparing a dataset for modelling

As we're mostly interested in the moment of en masse take-off of birds, we'll subset a dataset which contains only the moments that the measured `VIR` is increasing, a period that spans 23:00 - 23:20 CET.
...
mdl_variables <- c("VIR", "datetime", "dist_radar", "x", "y", "landuse", "dist_urban", "total_biomass", "avg_bodymass", "disturb_pot")
log10_variables <- c("VIR", "dist_urban", "pop", "nr_birds", "total_biomass", "disturb_pot")
# log10_variables <- c("VIR")
factor_variables <- c("landuse", "datetime")

prepare_modelling_dataset <- function(x, variables, dt_start, dt_end, VIR_minimum = 0) {
  x %>%
    mutate(disturb_pot = pop / dist_urban) %>%
    ...
    ungroup() %>%
    mutate_at(vars(all_of(factor_variables)), factor) %>%
