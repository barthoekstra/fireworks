...
4. The human population density in the vicinity of birds.
5. The distance to the nearest urban area.
...
We use the waterbird count from January 2018, and the point-transect counts from 2017, as these are the best in terms of coverage. We calculate the `total_biomass` parameter first, because we need to spread these out over the number of PPI pixels that are contained within a count when we join the counts and PPIs.
...
wb_raw %>%
  ...
  summarise(total_biomass = mean(total_biomass)) -> wb
...
ptt_raw %>%
  ...
  summarise(total_biomass = mean(total_biomass)) -> ptt
```
...
log10_variables <- c("VIR", "dist_urban", "human_pop", "total_biomass", "avg_bodymass", "dist_urban", "disturb_pot")
...
prepare_modelling_dataset <- function(x, variables, dt_start, dt_end, VIR_minimum = 0, min_sample_size = 50) {
  x %>%
    ...
    ungroup() %>%
    group_by(landuse_class, datetime) %>%
    mutate(nr_cells = n()) %>%
    filter(nr_cells > min_sample_size) %>%
    ungroup() %>%
...
predictors <- mdl_variables[!mdl_variables %in% c("VIR", "landuse_class", "landuse", "datetime", "pixel", "x", "y")]
corr_radar <- grouped_ggcorrmat(radar_cleaned, output = "plot", type = "spearman", cor.vars = all_of(predictors), grouping.var = "radar",
                                colors = c("#2166AC", "#F7F7F7", "#B2182B"), title.prefix = "Radar")
corr_radar
```
