...
for (scan in fireworks_scans) {
  ...
  calculate_vp(scan, vpfile = vpfile_out, verbose = FALSE, mount = dirname(fireworks_scans[1]))
  i = i + 1
}
```
...
```{r}
fireworks_vpfilepaths <- Sys.glob(file.path("data/processed/vp/fireworks-2017-2018", "*NL61*"))
...
ggplot(integrated[indexes, ], aes(x = datetime)) + 
  ...
  labs(title = "Time series of Vertically Integrated Reflectivities (VIR)",
       subtitle = "NYE 2017-2018, Herwijnen Radar",
       x = "Time (CET)",
       y = "VIR",
       colour = NULL) +
