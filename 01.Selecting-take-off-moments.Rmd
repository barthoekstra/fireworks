...
The plot shows $\Delta VIR/{scan}$ peaks much stronger and earlier for the Den Helder radar than for Herwijnen, which matches with the more pronounced change in the previous plot of the `vpts`.

We can now identify which scans 'contain' the peak moment of take-off. The datetime values correspond with the start times of the scans and the filenames of the polar volume files.

```{r identify_takeoff_scans}
max_vir_deriv[, 2:4]
```

To illustrate the processing workflow, we focus on these peak moments of take-off, so we store their identifiers for later use.
