...
#' map(my_composite, bm)
#' }
comp_ppi <- function(x, param = "DBZH", nx = 100, ny = 100, xlim, ylim, res, crs, raster = NA, method = "max", idp = 2, idw_max_distance=NA) {
  if (FALSE %in% sapply(x, is.ppi)) {
    stop("'composite' expects objects of class ppi only")
  }
  if (!is.numeric(nx) && missing(res)) stop("'nx' should be an integer")
  if (!is.numeric(ny) && missing(res)) stop("ny' should be an integer")
  if (!missing(xlim)) {
