...
#'
#' @export
gbm.bootstrap.functions.modified <- function(
  ...
  verbose = T)                # control reporting
{
  ...
  rel.infs <- array(0, dim = c(n.preds, n.reps))
  ...
  rownames(rel.infs) <- boot.model.gbm$var.names
  final.object$rel.infs <- rel.infs
