...
ppi_hrw$data@data %<>%
  left_join(dplyr::select(wb, area_nr, total_biomass, weighted_mean_weight), by = c("wb_area_nr" = "area_nr")) %>%
  ...
  ungroup() %>%
  left_join(dplyr::select(ptt, route, total_biomass, weighted_mean_weight), by = c("ptt_route" = "route")) %>%
