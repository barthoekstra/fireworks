...
The following table is used to convert/reclassify the classes contained within the `CLC2018` dataset, with the original land use classes under `landuse_class` and how they will be reclassified under `landuse_target`.
...
```{r}
landuse_crop_dhl <- ratify(reclassify(landuse_crop_dhl, cbind(landuse_classes$landuse_id, landuse_classes$landuse_target_id)), count = TRUE)
landuse_crop_hrw <- ratify(reclassify(landuse_crop_hrw, cbind(landuse_classes$landuse_id, landuse_classes$landuse_target_id)), count = TRUE)
```

This leaves us with the following count of cells per land use category

```{r}
cellcounts <- cbind(levels(landuse_crop_hrw)[[1]]["COUNT"], levels(landuse_crop_dhl)[[1]]["COUNT"])
...
# Rasterize
pop_density_rasterized_hrw <- st_rasterize(cbs_hrw["INWONER"],
                                           template = st_as_stars(st_bbox(ppi_hrw$data), dx = ppi_hrw$data@grid@cellsize[1], dy = ppi_hrw$data@grid@cellsize[2], 
                                                                  values = NA_real_))
pop_density_rasterized_dhl <- st_rasterize(cbs_dhl["INWONER"],
                                           template = st_as_stars(st_bbox(ppi_dhl$data), dx = ppi_dhl$data@grid@cellsize[1], dy = ppi_dhl$data@grid@cellsize[2],
                                                                  values = NA_real_))
```

```{r verify_population_density_rasters, results='hold'}
compareRaster(as(pop_density_rasterized_hrw, "Raster"), as(ppi_hrw$data, "RasterLayer"), extent = TRUE, rowcol = TRUE, crs = TRUE, res = TRUE, orig = TRUE)
compareRaster(as(pop_density_rasterized_dhl, "Raster"), as(ppi_dhl$data, "RasterLayer"), extent = TRUE, rowcol = TRUE, crs = TRUE, res = TRUE, orig = TRUE)
```

We can now calculate the total population in an area within ~2.5km around the center point.

```{r calculate_population_within_window_around_centerpoint}
weights <- matrix(1, nrow = 11, ncol = 11)
pop_density_rasterized_hrw[pop_density_rasterized_hrw < 0] <- 0
pop_density_rasterized_dhl[pop_density_rasterized_dhl < 0] <- 0
pop_hrw <- focal(as(pop_density_rasterized_hrw, "Raster"), w = weights, fun = sum, na.rm = TRUE)
pop_dhl <- focal(as(pop_density_rasterized_dhl, "Raster"), w = weights, fun = sum, na.rm = TRUE)
```

And add it to the PPIs.

```{r add_total_population_to_ppis}
values_pop_hrw <- as.data.frame(pop_hrw@data@values)
values_pop_dhl <- as.data.frame(pop_dhl@data@values)

ppi_hrw$data$pop <- as.numeric(unlist(values_pop_hrw))
ppi_dhl$data$pop <- as.numeric(unlist(values_pop_dhl))
```
...
```{r total_vir_vs_land_use_type, fig.show='hold', dev='png'}
...
```{r distance_to_urban_vs_vir, warning=FALSE, fig.show='hold', dev='png'}
ggplot(ppi_dhl$data@data, aes(x = dist_urban, y = VIR, colour = landuse_class)) + 
  geom_point(alpha = 0.5) + ylim(c(0, 500000)) + theme(legend.position = "none") +
...
That seems alright, so we can save the PPIS we have now annotated.
...
mapview(ppi_dhl$data[, , c("pop", "dist_urban")], alpha.regions = 0.6, col.regions = inferno, maxpixels=2000000,
        na.color = "#00000000", map.types = c("CartoDB.Positron", "CartoDB.DarkMatter", "Esri.WorldImagery"),
        layer.name = c("pop", "dist_urban"))
```
