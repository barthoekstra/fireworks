...
bootstraps <- mcmapply(function(m, g) gbm.bootstrap.functions.modified(m, list.predictors = g, n.reps = 200, n.divisions = 1000),
...
z_lims <- c(-3, 3)
pal <- colorNumeric(pal_resid, domain = z_lims, na.color = NA)
raster_resid <- as(ppi$data["resid"], "RasterLayer")

leaflet() %>%
  addTiles() %>%
  addRasterImage(raster_resid, colors = pal, layerId = "resid") %>%
  addImageQuery(raster_resid, layerId = "resid") %>%
  addLegend(pal = pal, values = z_lims) %>%
  addOpacitySlider(layerId = "resid") %>%
  addLayersControl(overlayGroups = "resid", position = "topleft") %>%
  identity()
```
