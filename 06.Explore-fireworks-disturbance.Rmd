...
```{r}
plot(ppi_hrw, param = "ptt_nr_birds", zlim = c(min(ppi_hrw$data@data$ptt_nr_birds, na.rm = TRUE), max(ppi_hrw$data@data$ptt_nr_birds, na.rm = TRUE)))
plot(ppi_hrw, param = "wb_nr_birds", zlim = c(min(ppi_hrw$data@data$wb_nr_birds, na.rm = TRUE), max(ppi_hrw$data@data$wb_nr_birds, na.rm = TRUE)))
plot(ppi_hrw, param = "birds_smooth_9", zlim = c(min(ppi_hrw$data@data$birds_smooth_9, na.rm = TRUE), max(ppi_hrw$data@data$birds_smooth_9, na.rm = TRUE)))
plot(ppi_hrw, param = "birds_smooth_15", zlim = c(min(ppi_hrw$data@data$birds_smooth_15, na.rm = TRUE), max(ppi_hrw$data@data$birds_smooth_15, na.rm = TRUE)))
...
ppi_hrw_mass %>%
  filter(VIR > 50 & birds_smooth_9 > 0 & wb_nr_birds > 0 & type == "rural" & VIR < quantile(ppi_hrw_mass$VIR, 0.99, na.rm = TRUE)) %>%
...
m1 <- gam(VIR ~ s(birds_smooth_9, k = 100) + s(pop, k = 20) + s(dist_urban) + landuse_overall,
