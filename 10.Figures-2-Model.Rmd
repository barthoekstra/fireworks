...
pairwise.t.test(all$vi, all$variable)
```

Indeed, mean variable importance for `dist_urban` is not significantly different from `total_biomass` and the same applies to `waterbodies` and `wetlands`.
...
x_labels <- c("urban" = "Urban", "agricultural" = "Agricultural", "semiopen" = "Semi-open", "forests" = "Forests", "wetlands" = "Wetlands", "waterbodies" = "Water bodies", "dist_urban" = "Distance to fireworks (m)", "total_biomass" = "Bird biomass (kg/km^2)", "dist_radar" = "Distance from radar (m)")
...
manual_limits <- list("dist_urban" = c(min(data_cleaned$dist_urban), quantile(data_cleaned$dist_urban, probs = 0.975)), 
                      "total_biomass" = c(0, quantile(data_cleaned$total_biomass, probs = 0.975)))
ylims <- c(-1.8, 1.5)
...
colors <- c("urban" = "#94346E", "agricultural" = "#73AF48", "semiopen" = "#EDAD08", "forests" = "#0F8554", "wetlands" = "#38A6A5", "waterbodies" = "#1D6996",
            "dist_urban" = "#94346E", "total_biomass" = "#CC503E", "dist_radar" = "#666666")
x_labels <- c("urban" = "Urban", "agricultural" = "Agricultural", "semiopen" = "Semi-open", "forests" = "Forests", "wetlands" = "Wetlands", "waterbodies" = "Water bodies", "dist_urban" = "Distance to fireworks (m)", "total_biomass" = "Bird biomass (kg/km^2)", "dist_radar" = "Distance from radar (km)")
limit_quantiles <- c("dist_urban", "total_biomass")
manual_limits <- list("dist_urban" = c(min(data_cleaned$dist_urban), max(data_cleaned$dist_urban)), "total_biomass" = c(min(data_cleaned$total_biomass), max(data_cleaned$total_biomass)), "dist_radar" = c(0, 66000))
ylims <- c(-4, 3)
...
plot_mboost_pdp <- function(modelci, data, which, confints, colors = NULL, ylims = ylims, varimp = NULL) {
  ...
  if (which == "dist_urban") {
    p <- p +
      annotate("text", x = manual_limits$dist_urban[[2]] * offset_right, y = ylims[2] - offset_top, label = vi_string, hjust = 1, color = colors[which],
               fontface = "bold", size = 5, alpha = 0.5)
  } else if(which == "total_biomass") {
    p <- p +
      annotate("text", x = manual_limits$total_biomass[[2]] * offset_right, y = ylims[2] - offset_top, label = vi_string, hjust = 1, color = colors[which],
               fontface = "bold", size = 5, alpha = 0.5)
  } else if(which == "dist_radar") {
    p <- p +
      annotate("text", x = manual_limits$dist_radar[[2]] * offset_right, y = ylims[2] - offset_top, label = vi_string, hjust = 1, color = colors[which],
               fontface = "bold", size = 5, alpha = 0.5)
  } else {
    p <- p +
  ...
  biomass_formatter <- function(x) x * 4
  ...
  if (which == "dist_urban") {
    ...
    p <- p +
      coord_cartesian(xlim = manual_limits$total_biomass, ylim = ylims, expand = FALSE) +
      scale_x_continuous(labels = biomass_formatter)
  } else if (which == "dist_radar") {
    xlabel <- x_labels[which]
    p <- p +
