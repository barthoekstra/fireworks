...
We use `renv` to make this environment reproducible. See [renv documentation](https://rstudio.github.io/renv/articles/renv.html) on how to setup a similar environment.
```{r warning=FALSE}
library(bioRad)
library(plotly)
...
### 2.5.3 Remove dynamic and static clutter
...
First we calculate the vertical profile using the `vol2bird` algorithm [@dokter_bird_2011].
