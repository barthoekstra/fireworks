...
data %>%
  ...
  mutate(across(all_of(log10_variables), log10)) %>%
  filter_all(all_vars(is.finite(.))) %>%
  as.data.frame() %>%
  droplevels() -> data_cleaned
```
...
For the timestamps we have now selected, we can train `gbm` models to determine for which timestamp it performs best.
...
plot_pdp <- function(model, predictor) {
  p <- partial(model, train = model$gbm.call$dataframe, pred.var = predictor, type = "regression", plot = TRUE, plot.engine ="ggplot2",
               smooth = TRUE, rug = TRUE, n.trees = model$n.trees, x)
  p
}
...
for (model in models) {
  ...
  plots <- lapply(model$var.names, function(x) {plot_pdp(model = model, predictor = x)})
