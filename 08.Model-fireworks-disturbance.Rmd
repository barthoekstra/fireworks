...
The main hyperparameter to be tuned for the boosted GAMs using `mboost` is the number of boosting iterations/steps. By stopping boosting before model performance (measured using cross-validation) worsens, we can avoid overfitting.
...
saveRDS(cvm, file = "data/models/cvm.RDS")
```

```{r plot_boosting_steps}
cvm <- readRDS("data/models/cvm.RDS")
plot(cvm)
```
