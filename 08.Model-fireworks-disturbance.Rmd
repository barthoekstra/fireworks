...
As we want to correct for the influence of `total_rcs` on the measured response by the radars, we will test the performance of a simple model using just `dist_radar` to correct for range-biased measurement error and `total_rcs` for the resolutions we have generated composte PPIs for so far (500m, 1000m and 2000m).
...
```{r calculate_autocovariate, eval=full_repro}
...
Having calculated the autocovariate, we can include it in our dataset and retrain a model, this time with the inclusion of the `autocov` parameter.
...
rownames(perf) <- c("dist_urban_rac", "dist_urban_original")
perf
```
