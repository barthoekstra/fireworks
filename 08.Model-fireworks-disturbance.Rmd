...
Furthermore, we want to make sure the area included is at least 'covered' by 1 radar, we have an estimate of total_biomass for these sites and the proportion of urban area contained within the PPI pixels is less than .25.
...
overall_models <- mclapply(x_vars, function(x) gbm.step(data = data_cleaned, gbm.y = 1, gbm.x = x, bag.fraction = 0.75, family = "gaussian", tree.complexity = 2, learning.rate = lr, silent = silence))

ggPerformance("dist_urban" = overall_models[[1]], "human_pop" = overall_models[[2]], "disturb_pot" = overall_models[[3]])
```
...
datasets <- lapply(datetimes, function(x) filter(data_cleaned, datetime >= x & datetime < x + 5 * 60))

models <- mclapply(datasets, function(x) {
  gbm.step(data = x, gbm.y = 1, gbm.x = match(c(base_model, "disturb_pot"), colnames(x)), bag.fraction = 0.75, family = "gaussian", 
           tree.complexity = 2, learning.rate = lr, tolerance.method = "fixed", tolerance = 8e-4, plot.main = TRUE, verbose = FALSE)
}, mc.preschedule = TRUE, mc.silent = FALSE)
...
perf <- do.call(ggPerformance, models)
